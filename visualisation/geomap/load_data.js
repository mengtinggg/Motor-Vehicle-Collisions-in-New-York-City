...
const load_data = d3.csv(DATA_URL).then(function(d){
    ...
    for (var i = 0; i < d.length; i++) { 
        if (d[i]['LOCATION'] != '' && d[i]['BOROUGH'] != ''){
            ...
            // if (d[i]['COLLISION_ID'] == '4026515'){
            //     console.log(d[i])
            //     console.log(d[i]['NUMBER OF PERSONS KILLED'])
            // }

            d[i]['LATITUDE'] = parseFloat(d[i]['LATITUDE'])
            d[i]['LONGITUDE'] = parseFloat(d[i]['LONGITUDE'])
            d[i]['NUMBER OF CYCLIST INJURED'] = (!!d[i]['NUMBER OF CYCLIST INJURED']) ? parseInt(d[i]['NUMBER OF CYCLIST INJURED']) : 0
            d[i]['NUMBER OF CYCLIST KILLED'] = (!!d[i]['NUMBER OF CYCLIST KILLED']) ? parseInt(d[i]['NUMBER OF CYCLIST KILLED']) : 0
            d[i]['NUMBER OF MOTORIST INJURED'] = (!!d[i]['NUMBER OF MOTORIST INJURED']) ? parseInt(d[i]['NUMBER OF MOTORIST INJURED']) : 0
            d[i]['NUMBER OF MOTORIST KILLED'] = (!!d[i]['NUMBER OF MOTORIST KILLED']) ? parseInt(d[i]['NUMBER OF MOTORIST KILLED']) : 0
            d[i]['NUMBER OF PEDESTRIANS INJURED'] = (!!d[i]['NUMBER OF PEDESTRIANS INJURED']) ? parseInt(d[i]['NUMBER OF PEDESTRIANS INJURED']) : 0
            d[i]['NUMBER OF PEDESTRIANS KILLED'] = (!!d[i]['NUMBER OF PEDESTRIANS KILLED']) ? parseInt(d[i]['NUMBER OF PEDESTRIANS KILLED']) : 0
            d[i]['NUMBER OF PERSONS INJURED'] = (!!d[i]['NUMBER OF PERSONS INJURED']) ? parseInt(d[i]['NUMBER OF PERSONS INJURED']) : 0
            d[i]['NUMBER OF PERSONS KILLED'] = (!!d[i]['NUMBER OF PERSONS KILLED']) ? parseInt(d[i]['NUMBER OF PERSONS KILLED']) : 0
